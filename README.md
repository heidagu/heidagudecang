...
![磁盘镜像页](screenshot-disc.png)

## 功能特性

- **批量并行转换**：一次添加多个文件，自动用满 CPU 核心（并发数可调，0 = 自动取核心数 − 1）
- **磁盘镜像工具**：文件夹一键打包成 ISO（全平台），从 ISO / IMG / CDR 等镜像提取内容
...
> 注意：ffmpeg 各构建（LGPL/GPL）有其各自许可证，与 VConv 项目本身（MIT）相互独立。ISO 打包依赖 [pycdlib](https://github.com/clalancette/pycdlib)（LGPL 2.1，作为 pip 依赖安装）。

## 使用说明

0. 顶部标签页切换「视频转换 / 磁盘镜像」：镜像页选文件夹 →「开始打包为 ISO」；或选 ISO/IMG 镜像文件 →「开始提取」（默认解到镜像旁自动新建的「xxx-提取」文件夹）
